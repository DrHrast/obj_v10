...
namespace all_tests
{
	...
	public:
		TEST_METHOD(test_01)
		{
			...
			// DID_IT: read values from input stream into vector
		...
		TEST_METHOD(test_02a)
		{
			...
			// DID_IT: fill vector with incremental values
		...
		TEST_METHOD(test_02b)
		{
			...
			// DID_IT: fill vector with incremental values (by 2)
			//auto f = [n=-1, m=2] (int x) mutable {return n += m + x; };
		...
		TEST_METHOD(test_03a)
		{
			...
			// DID_IT: change all values in a vector
			std::transform(v.cbegin(), v.cend(), v.begin(), [](int n) {return pow(n, 3); });
		...
		TEST_METHOD(test_03b)
		{
			...
			// DID_IT: calculate distances from origin (from x and y collections) to new vector
			std::transform(std::begin(x), std::end(x), y.begin(), std::back_inserter(d), [](int a, int b) {return std::sqrt(a * a + b * b); });
		...
		TEST_METHOD(test_04a)
		{
			...
			auto res = std::accumulate(it, end, 0.); // DID_IT: sum of all values in input stream
		...
		TEST_METHOD(test_04b)
		{
			...
			auto res = std::accumulate(v.begin(), v.end(), std::string("GO ")); // DID_IT: concatenated string with additional prefix 
		...
		TEST_METHOD(test_04c)
		{
			...
			auto total_age = std::accumulate(v.begin(), v.end(), 0, [](int age, person const& p) {return age + p.age; }); // DID_IT: sum of all ages
		...
		TEST_METHOD(test_05a)
		{
			...
			auto number_of_negative = std::count_if(v.begin(), v.end(), [](int n) {return n < 0; }); // DID_IT: 
		...
		TEST_METHOD(test_05b)
		{
			...
			auto number_of_invalid = std::count(v.begin(), v.end(), 1e10); // DID_IT: 
		...
		TEST_METHOD(test_05c)
		{
			...
			auto number_in_first_quadrant = std::count_if(v.begin(), v.end(), [](point const& p) {return p.x > 0 && p.y > 0; }); // DID_IT: 
		...
		TEST_METHOD(test_07a)
		{
			...
			// DID_IT: change every invalid value (1e10) with -1 
		...
		TEST_METHOD(test_07b)
		{
			...
			// DID_IT: change every vowel with x 
			std::replace_if(s.begin(), s.end(), [](char sl, std::string l = "aeiou") {return l.find(sl) != std::string::npos; }, 'x');
		...
		TEST_METHOD(test_08a)
		{
			...
			// DID_IT: delete all invalid values (1e10)
		...
		TEST_METHOD(test_08b)
		{
			...
			// DID_IT: delete all vowels 
			auto it = std::remove_if(s.begin(), s.end(), [](char sl, std::string l = "aeiou") {return l.find(sl) != std::string::npos; });
			s.erase(it, s.end());
		...
		TEST_METHOD(test_09)
		{
			...
			// DID_IT: sort vector by grade, then by points
			std::sort(v.begin(), v.end(), [](exam const& g, exam const& p) {
				if (g.grade != p.grade)
					return g.grade > p.grade;
		...
		TEST_METHOD(test_10)
		{
			...
			// DID_IT: put median value in the middle of vector. fast.
		...
		TEST_METHOD(test_11)
		{
			...
			auto smallest_value = *std::min_element(v.begin(), v.end());// DID_IT: 
			Assert::AreEqual(-97.23, smallest_value);
			auto largest_value = *std::max_element(v.begin(), v.end());// DID_IT: 
		...
		TEST_METHOD(test_12)
		{
			...
			std::adjacent_difference(atp_points.begin(), atp_points.end(), std::back_inserter(diff));// DID_IT: 
